...
#' @description  A Bayesian Multistate Life Table Method for survey data, developed by Lynch and Zang (2022), allowing for large state spaces with quasi-absorbing states (i.e., structural zeros in a transition matrix). This function came from the deprecated \code{bayeslogit} package, which conducts Bayesian multinomial logistic regressions using Polya-Gamma latent variables (Polson 2013). It should be jointly used with the \code{mlifetable()} function, which will generate life tables based on the estimates from regressions. 
...
#' @param thining The thinning strategy to reduce autocorrelation. For example, if \code{thining = 5}, this function will select 1 from every 5 posterior samples and generate a new dataset named \code{outwstepwidth.txt}. Default is 5.
...
#'   \item \strong{outwstepwidth}: An array generated by selecting one sample from every \emph{thining} samples in \strong{out}.
#' }
